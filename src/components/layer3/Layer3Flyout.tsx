...
export function Layer3Flyout({ data, fields, isOpen, onClose, t }: FlyoutProps) {
  ...
  const StatRow = ({ label, stats, isSrc = true }: any) => (
    <div className="flex items-center py-1 px-2 border-b border-slate-800/30 last:border-0">
      ...
      <div className="flex-1 flex gap-6 text-[11px] font-mono">
      ...
      <div className={cn(
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-2 space-y-4">
          <div className="space-y-0.5 bg-slate-900/20 p-2 rounded-xl border border-slate-800/50 shadow-inner">
            ...
            <ArkimeRow label="PROTOCOLS" value={data.protocols?.length > 0 ? data.protocols.join(" ") : data.protocol} color="text-blue-400 font-bold" />
                  ...
                  data.tags?.length > 0 && data.tags[0] !== undefined ? (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {data.tags.map((tag: string) => (
                        <span key={tag} className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 rounded text-[9px] font-bold text-slate-400">{tag}</span>
                      ))}
                    </div>
                  ) : "-" 
                } />
          ...
          {data.ssh && (
            <div className="space-y-0.5 bg-blue-500/5 p-2 rounded-xl border border-blue-500/20 shadow-inner animate-in fade-in slide-in-from-bottom-2">
              <div className="text-[10px] font-bold text-blue-400 px-2 py-1 mb-2 uppercase tracking-widest border-b border-blue-500/10 bg-blue-500/10 rounded-t">
                {t?.sshInfo || "SSH INFORMATION"}
              </div>
