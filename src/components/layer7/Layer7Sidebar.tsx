    ...
    <div className="group border-b border-slate-900/30 last:border-0">
      ...
      >
        ...
        <button
          ...
          className={cn(
            "p-1 hover:bg-slate-700 rounded-md transition-opacity",
                ...
                <div className="space-y-4">
                  {!stats?.buckets || stats.buckets.length === 0 ? (
                    ...
                    <>
                      ...
                      <div
                        className={cn(
                          "flex items-end h-28 pt-2 relative",
                          stats.buckets.length > 40 ? "gap-0" : "gap-[1.5px]",
                        )}
                      >
                        {(() => {
                          const maxCount = Math.max(
                            ...stats.buckets.map((b: any) => b.doc_count),
                          );
                          return stats.buckets.map(
                            (bucket: any, idx: number) => {
                              const height =
                                (bucket.doc_count / (maxCount || 1)) * 100;
                              return (
                                <div
                                  key={idx}
                                  className="flex-1 min-w-[1px] bg-blue-500/50 hover:bg-blue-400 transition-all rounded-t-[1px] relative group/bar"
                                  ...
                                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover/bar:block z-50">
                                    <div className="bg-slate-800 border border-slate-700 text-[10px] text-white px-2 py-1.5 rounded shadow-2xl font-mono">
                                      ...
                                      <div className="font-bold text-blue-400">
                                        {bucket.doc_count.toLocaleString()} hits
                                      </div>
                                    </div>
                                  </div>
                                </div>
                      ...
                      <div className="flex justify-between text-[10px] text-slate-400 font-mono px-1 border-t border-slate-800/30 pt-2 tracking-tighter">
                        ...
                        <span className="opacity-50">
                          {formatTime(
                            stats.buckets[Math.floor(stats.buckets.length / 2)],
                          )}
                        </span>
                ...
                <div className="space-y-2">
                  {stats?.buckets
                    ...
                    .map((bucket: any, idx: number) => {
                      const percentage =
                        (bucket.doc_count / (stats.total || 1)) * 100;
                      return (
                        <div key={idx} className="space-y-1">
                          <div className="flex justify-between text-[10px] gap-2 items-center">
                            <span className="text-slate-300 truncate font-mono flex-1">
                              {bucket.key || "empty"}
                            </span>
