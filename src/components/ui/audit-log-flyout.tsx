...
const flattenObject = (obj: any, prefix = ""): Record<string, any> => {
  ...
  const ignoredRootKeys = [
    "id", "user_name", "id_type", "role", 
    "action", "path", "resource", "status_code", "client_ip"
  ];

  for (const key in obj) {
    if (!prefix && ignoredRootKeys.includes(key)) continue;
...
export function AuditLogFlyout({
  event,
  events = [],
  currentIndex = -1,
  onNavigate,
          ...
          {events.length > 0 && (
            <div className="flex items-center text-slate-400 text-[10px] sm:text-[11px] font-medium gap-1 bg-slate-900 rounded-md border border-slate-800 p-0.5 select-none">
              ...
              <span className="px-2 min-w-[50px] sm:min-w-[70px] text-center tracking-tight">
                {currentIndex >= 0
                  ? `${currentIndex + 1} ${dict.paginationOf || "of"} ${events.length}`
                  : `0 ${dict.paginationOf || "of"} 0`}
              </span>
              <button
