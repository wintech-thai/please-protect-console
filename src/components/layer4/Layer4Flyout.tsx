...
export function Layer4Flyout({
  ...
  return (
    <>
      <div className={cn(
        "fixed top-0 right-0 h-screen w-full max-w-[850px] bg-slate-950 border-l border-slate-800 shadow-[-20px_0_60px_rgba(0,0,0,0.7)] z-[100] flex flex-col transition-transform duration-300 ease-in-out",
          ...
          {drawerTab === "table" ? (
            <div className="flex flex-col h-full">
              ...
              <div className="flex-none flex border-b border-slate-800 bg-slate-900/50 text-[10px] font-bold text-slate-500 uppercase tracking-widest select-none">
                <div className="w-[45%] sm:w-[35%] px-4 py-2 border-r border-slate-800">{t?.flyout?.field || "FIELD"}</div>
                <div className="w-[55%] sm:w-[65%] px-4 py-2">{t?.flyout?.value || "VALUE"}</div>
              </div>
                  ...
                  .map((f, idx) => {
                    ...
                    return (
                      <div key={idx} className="group flex border-b border-slate-900 hover:bg-slate-900/30 transition-colors text-sm relative">
                        <div className="w-[45%] sm:w-[35%] px-4 py-2.5 border-r border-slate-900 flex flex-col sm:flex-row sm:items-center justify-between min-w-0 group/field">
                        ...
                        <div className="w-[55%] sm:w-[65%] px-4 py-2 flex items-center min-w-0 break-all">
                          {renderValueCell(f)}
                        </div>
                      </div>
                    );
