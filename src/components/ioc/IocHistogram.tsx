        ...
        {data.map((bucket, i) => {
          ...
          return (
            <div key={i} className="flex-1 min-w-0 max-w-[40px] h-full flex flex-col justify-end group relative cursor-pointer mx-auto">
              ...
              {/* Hover Highlight Bar */}
              <div className="absolute inset-y-0 -inset-x-1 bg-rose-500/[0.05] hidden group-hover:block z-0 pointer-events-none rounded-sm" />
                ...
                {allBuckets.map((sub: any) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: getDynamicTypeColor(sub.key),
                    }}
                    className={cn(
