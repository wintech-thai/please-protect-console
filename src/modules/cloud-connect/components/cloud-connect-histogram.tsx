...
const getDatasetColor = (dataset: string | number) => {
  const status = parseInt(String(dataset), 10);
  if (status >= 200 && status < 300) return "#10b981"; // Success (Green)
  if (status >= 300 && status < 400) return "#3b82f6"; // Redirect (Blue)
  if (status >= 400 && status < 500) return "#f59e0b"; // Client Error (Orange)
  if (status >= 500 && status < 600) return "#ef4444"; // Server Error (Red)
  return "#64748b"; // Unknown (Gray)
};

export function CloudConnectHistogram({
  data,
  totalHits,
  interval,
  maxDocCount,
  ...
  return (
    <div className="flex-none h-48 bg-slate-900/50 border border-slate-800 rounded-xl rounded-b-none p-4 pb-10 flex flex-col backdrop-blur-sm relative z-0 select-none">
      {/* Header */}
      <div className="flex items-center justify-between mb-4">
        <div className="text-sm font-bold text-white tracking-tight">
          {totalHits.toLocaleString()}{" "}
          <span className="text-slate-500 font-normal ml-1">hits</span>
        </div>
      ...
      >
        {data.map((bucket, i) => {
          ...
          return (
            ...
            >
              {/* Highlight Column */}
              <div className="absolute inset-y-0 inset-x-0 bg-blue-500/5 hidden group-hover:block z-0 pointer-events-none" />
              ...
              >
                ...
                >
                  ...
                  <div className="text-[10px] text-blue-400 font-bold font-mono text-center border-b border-slate-800 pb-2 mb-2">
                    {startTime.format("D MMM YYYY HH:mm:ss")} -{" "}
                  ...
                  <div
                    className={cn(
                      "max-h-45 overflow-y-auto no-scrollbar",
                      isGrid
