...
interface CloudConnectLogFlyoutProps {
  ...
  onClose: () => void;
  dict: typeof cloudConnectDict.EN.flyout;
}

export const getFieldIcon = (field: string) => {
  const f = field.toLowerCase();
  if (f.includes("@timestamp") || f.includes("time")) return <Calendar className="w-3 h-3" />;
  if (f.includes("ip") || f.includes("domain") || f.includes("host") || f.includes("path")) return <Globe className="w-3 h-3" />;
  if (f.includes("port") || f.includes("id") || f.includes("status") || f.includes("latency")) return <Hash className="w-3 h-3" />;
...
export function CloudConnectLogFlyout({ log, onClose, dict }: CloudConnectLogFlyoutProps) {
  ...
  return (
    <>
      <div
        ...
        onClick={onClose}
      />
      <div className="fixed inset-y-0 right-0 w-full sm:w-162.5 bg-slate-950 border-l border-slate-800 shadow-[0_0_60px_rgba(0,0,0,0.5)] z-50 flex flex-col animate-in slide-in-from-right duration-300 font-sans">
        <div className="flex-none px-6 py-4 border-b border-slate-800 bg-slate-950 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <h3 className="text-sm font-bold text-white tracking-widest opacity-80">
              {dict.title}
            </h3>
          </div>
              ...
              <div className="flex-1 overflow-auto custom-scrollbar pb-10">
                {sortedKeys
                  ...
                  .map((k) => {
                    ...
                    const displayKey = k;
                    const icon = getFieldIcon(displayKey);
