...
export default function SubnetMappingPage() {
  const { language } = useLanguage();
  const t = subnetTranslations.subnetList[language as keyof typeof subnetTranslations.subnetList] 
           || subnetTranslations.subnetList.EN;
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [data, countData] = await Promise.all([
        subnetApi.getSubnets({ offset, limit: itemsPerPage, fullTextSearch: activeSearch }),
        subnetApi.getSubnetCount({ fullTextSearch: activeSearch })
      ]);

      let loadedSubnets: any[] = [];
      if (Array.isArray(data)) loadedSubnets = data;
      else if (data?.data) loadedSubnets = data.data;

      const mapped: SubnetData[] = loadedSubnets.map((s: any) => ({
        id: s.id,                
        cidr: s.cidr,            
        department: s.name,      
        ipCount: calculateIPCount(s.cidr),
        tags: s.tags ? s.tags.split(',').filter((t: string) => t.trim() !== '') : []
      }));
  ...
  }, [isLoading, selectedRowId, subnets]);
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(selectedIds.map(id => subnetApi.deleteSubnet(id)));
      ...
      toast.success(t.toast.deleteSuccess.replace("{count}", deleteCount.toString())); 
      setSelectedIds([]);
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) setSelectedIds(subnets.map(s => s.id));
    else setSelectedIds([]);
  };
      ...
      <div className="flex-none py-4">
        <div className="flex flex-col lg:flex-row gap-4 justify-between items-start lg:items-center bg-slate-900/50 p-4 rounded-xl border border-slate-800 shadow-sm">
            ...
            <div className="flex gap-2 w-full lg:w-auto justify-end">
                <Link 
                  href={selectedRowId ? `/events/subnet/create?prevHighlight=${selectedRowId}` : "/events/subnet/create"} 
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        <div className="flex-1 bg-slate-900 border border-slate-800 rounded-t-xl shadow-2xl overflow-hidden flex flex-col">
            <div className="flex-1 overflow-auto no-scrollbar">
                <table className="w-full text-left border-collapse min-w-[1000px]"> 
                    <thead className="bg-slate-950 sticky top-0 z-10 text-xs font-semibold text-slate-400 uppercase tracking-wider">
                        <tr>
                            <th className="p-4 border-b border-slate-800 w-[50px]"><input type="checkbox" onChange={handleSelectAll} checked={subnets.length > 0 && selectedIds.length === subnets.length} /></th>
                            <th className="p-4 border-b border-slate-800">{t.columns.cidr}</th>
                            <th className="p-4 border-b border-slate-800">{t.columns.department}</th>
                            <th className="p-4 border-b border-slate-800 ">{t.columns.totalIp}</th>
                            <th className="p-4 border-b border-slate-800">{t.columns.tags || "Tags"}</th> 
                            <th className="p-4 border-b border-slate-800 text-center w-[100px]">{t.columns.actions}</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800/50">
                        {isLoading ? (
                            <tr><td colSpan={6} className="p-20 text-center"><div className="flex flex-col items-center gap-3"><Loader2 className="w-8 h-8 animate-spin text-blue-500" /><span className="text-slate-500">{t.loading}</span></div></td></tr>
                        ) : subnets.length === 0 ? (
                            <tr><td colSpan={6} className="p-20 text-center text-slate-500">{t.noData}</td></tr>
                        ) : (
                            subnets.map((subnet, idx) => {
                                const isHighlighted = selectedRowId === subnet.id;

                                return (
                                    <tr 
                                        key={subnet.id || idx} 
                                        ref={(el) => { if (el) rowRefs.current[subnet.id] = el; }}
                                    ...
                                    >
                                        <td className="p-4" onClick={(e) => e.stopPropagation()}>
                                          <input type="checkbox" checked={selectedIds.includes(subnet.id)} onChange={() => handleSelectOne(subnet.id)} />
                                        </td>
                                        
                                        <td className="p-4 font-mono font-medium">
                                          <Link 
                                            href={`/events/subnet/${subnet.id}/update`}
                                        ...
                                        <td className="p-4 text-slate-300">{subnet.department}</td>
                                        
                                        <td className="p-4 text-emerald-400 font-mono ">
                                            {subnet.ipCount.toLocaleString()}
                                        </td>
                                        <td className="p-4">
                                            <div className="flex flex-wrap gap-1.5">
                                                {subnet.tags.length > 0 ? (
                                                    subnet.tags.map((tag, i) => (
                                                        <span key={i} className="px-2 py-0.5 rounded-full text-[10px] font-medium bg-blue-500/10 text-blue-400 border border-blue-500/20 whitespace-nowrap">
                                                            {tag}
                                                        </span>
                                                    ))
                                                ) : (
            ...
            <div className="flex-none flex items-center justify-between sm:justify-end px-4 py-3 border-t border-slate-800 bg-slate-950 z-20 gap-4 sm:gap-6">
                ...
                <div className="flex items-center gap-4">
                    <div className="text-xs text-slate-400">{totalCount === 0 ? '0-0' : `${startRow}-${endRow}`} {t.pagination.of} {totalCount}</div>
            ...
            >
                <div className="flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-slate-400 mb-5 leading-relaxed">
                        {t.deleteModal.message.replace("{count}", selectedIds.length.toString())}
                    </p>
